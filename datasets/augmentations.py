...
from utils.interpolation import Interp2, Interp2MaskBinary, Meshgrid
...
class RandomAffineFlow(nn.Module):
    ...
    def forward(self, example_dict):
        ...
        if self._addnoise:
            ...
            im1 = im1 + self._noise1
            im2 = im2 + self._noise2
            im1.clamp_(0.0, 1.0)
            im2.clamp_(0.0, 1.0)
...
class RandomAffineFlowOcc(nn.Module):
    def __init__(self, args, addnoise=True):
        ...
        self._random_mirror = RandomMirror()
        self._addnoise = addnoise
        self._crop = args.training_augmentation_crop
    ...
    def random_crop(self, im1, im2, flo_f, flo_b, occ1, occ2):
        ...
        crop_height = self._crop[0]
        crop_width = self._crop[1]
