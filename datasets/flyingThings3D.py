...
class FlyingThings3d(data.Dataset):
    def __init__(self,
        ...
        if flow_root != None and not os.path.isdir(flow_root):
            raise ValueError("Flow directory '%s' not found!")
        if occ_root != None and not os.path.isdir(occ_root):
        ...
        self._image_list = []
        self._flow_list = [] if flow_root != None else None
        self._occ_list = [] if occ_root != None else None
        ...
        if photometric_augmentations:
            # uint8 -> PIL
            aug_list = [vision_transforms.ToPILImage()]

            if args.training_augmentation_ColorJitter:
                # PIL -> PIL : random hsv and contrast
                aug_list.append(
                    vision_transforms.ColorJitter(brightness=args.training_augmentation_ColorJitter[0], 
                                                  contrast=args.training_augmentation_ColorJitter[1],
                                                  saturation=args.training_augmentation_ColorJitter[2],
                                                  hue=args.training_augmentation_ColorJitter[3]))
                # PIL -> FloatTensor
                aug_list.append(vision_transforms.transforms.ToTensor())

            if args.training_augmentation_RandomGamma:
                aug_list.append(
                    transforms.RandomGamma(min_gamma=args.training_augmentation_RandomGamma[0],
                                           max_gamma=args.training_augmentation_RandomGamma[1],
                                           clip_image=args.training_augmentation_RandomGamma[2]))

            self._photometric_transform = transforms.ConcatTransformSplitChainer(aug_list, from_numpy=True, to_numpy=False)
